<script>
  ...
  import { 
    ...
    stakeSwampers, 
    stakeCreatures, getUserInfo, getSwamperRewardPerDay, getCreatureRewardPerDay, getUnstakedSwampers, getUnstakedCreatures } from "./utils";
  ...
  const setInitialData = async () => {
    ...
    if(connected) {
      ...
      swampverseNfts = await fetchSwampverseNfts(wallet);
      creaturesNfts = await fetchCreaturesNfts(wallet);
    }
  }
  setInitialData();
  ...
  <SubHeader title="Inventory" subtitle="Stake Swampers & Creatures" backable />
  <br />
  ...
  <section>
    ...
    {#if (category === "swampers" && swampersApproved) || (category === "creatures" && creaturesApproved)}
      <button disabled={!selected.length} on:click={() => category === "creatures" ? stakeCreatures(selected, wallet) : stakeSwampers(selected, wallet)}>Stake Selected {category[0].toUpperCase() + category.substring(1)} ({selected.length})</button>
      <SelectableNft 
